...
def mandatory_tour_scheduling(tours,
    ...
    for spec_segment_name, spec_settings in spec_segment_settings.items():
        ...
        model_spec = simulate.read_model_spec(spec_file_name)
        coefficients_df = simulate.read_model_coefficients(spec_segment_settings[spec_segment_name])
        specs[spec_segment_name] = simulate.eval_coefficients(model_spec, coefficients_df)

        # estimator for this tour_segment
        estimator = estimation.manager.begin_estimation('mandatory_tour_scheduling_%s' % spec_segment_name)
        if estimator:
            estimators[spec_segment_name] = estimator  # add to local list
            estimator.write_model_settings(model_settings, model_settings_file_name)
            estimator.write_spec(file_name=spec_file_name)
    ...
    if estimators:
        timetable.begin_transaction(estimators)
    ...
    if estimators:
        ...
        for spec_segment_name, estimator in estimators.items():
            ...
            override_choices = estimator.get_override_choices(model_choices)
